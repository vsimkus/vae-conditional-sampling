...
from irwg.utils.weights import multiple_importance_logprob
...
#     latent_logprob = reduce(latent_logprob, '... c h w -> ...', 'sum')
#     dims_collapsed = 3
#     latent_logprob = torch.logsumexp(latent_logprob, dim=var_comp_neg_idx+dims_collapsed, keepdim=True)
#     latent_logprob -= torch.log(torch.tensor(Z.shape[var_comp_neg_idx], device=Z.device))

#     # Undo the swap axes above
#     latent_logprob = torch.swapaxes(latent_logprob, 0, var_comp_neg_idx+dims_collapsed).squeeze(0)
...
class PairedCellAR(nn.Module):
    ...
    def inverse(self, z, ftr):
        new_z, log_det2 = self.cell2.inverse(z, ftr)
        new_z, log_det1 = self.cell1.inverse(new_z, ftr)
...
class AutoEncoder(nn.Module):
    ...
    def forward(self, x, *, p_t=1.0, q_t=1.0):
        s = self.stem(2 * x - 1.0)
        ...
        dist = Normal(mu_q, log_sig_q, temp=q_t)   # for the first approx. posterior
        z, _ = dist.sample()
        ...
        # prior for z0
        dist = Normal(mu=torch.zeros_like(z), log_sigma=torch.zeros_like(z), temp=p_t)
        ...
        for cell in self.dec_tower:
            if cell.cell_type == 'combiner_dec':
                if idx_dec > 0:
                    ...
                    dist = Normal(mu_p + mu_q, log_sig_p + log_sig_q, temp=q_t) if self.res_dist else Normal(mu_q, log_sig_q, temp=q_t)
                    z, _ = dist.sample()
                    all_z0.append(z)
                    log_q_conv = dist.log_p(z)
                    # apply NF
                    ...
                    # evaluate log_p(z)
                    dist = Normal(mu_p, log_sig_p, temp=p_t)
                    log_p_conv = dist.log_p(z)
                    all_p.append(dist)
        ...
        return logits, log_q, log_p, kl_all, kl_diag, all_q, all_z0, all_z

    def forward_dmis(self, x, batch_shape, *, p_t=1.0, q_t=1.0):
        B = x.shape[0]
        s = self.stem(2 * x - 1.0)
        ...
        for n in range(self.num_flows):
            ...
            # Below is only correct for the additive flows
            assert not self.nf_cells[n].cell1.use_mix_log_cdf and not self.nf_cells[n].cell2.use_mix_log_cdf
        nf_offset += self.num_flows
        all_q = [dist]
        all_log_q = [log_q_conv]

        # NOTE: for the affine flows where the `ftr` is not used, we do not need to
        # reverse the flow, simply use what was z0, because the transforms only depend
        # on the z (and no additional features, e.g. `ftr`), and the log_det is always 0.
        # for n in reversed(range(self.num_flows)):
        #     z_rev, log_det = self.nf_cells.inverse[nf_offset-n](z_rev, ftr)
        # ...
        # z0 = z0.reshape(batch_shape + z0.shape[1:])
        # dist_z0 = Normal(mu_q.reshape(batch_shape + mu_q.shape[1:]), log_sig_q.reshape(batch_shape + log_sig_q.shape[1:]))
        # log_qz0 = multiple_importance_logprob(z0, dist_z0, var_comp_neg_idx=-4)
        ...
        for cell in self.dec_tower:
            if cell.cell_type == 'combiner_dec':
                if idx_dec > 0:
                    ...
                    z, _ = dist.sample()
                    all_z0.append(z)
                    all_q0_mu.append(mu_p + mu_q if self.res_dist else mu_q)
                    all_q0_logsig.append(log_sig_p + log_sig_q if self.res_dist else log_sig_q)
                    log_q_conv = dist.log_p(z)
                    # apply NF
                    for n in range(self.num_flows):
                        z, log_det = self.nf_cells[nf_offset + n](z, ftr)
                        log_q_conv -= log_det
                        # Below is only correct for the additive flows
                        assert not self.nf_cells[nf_offset + n].cell1.use_mix_log_cdf and not self.nf_cells[nf_offset + n].cell2.use_mix_log_cdf
                    nf_offset += self.num_flows
                    all_log_q.append(log_q_conv)
                    all_q.append(dist)
        ...
        all_z0_stacked = torch.concat([rearrange(z0, '... c h w -> ... (c h w)') for z0 in all_z0], dim=-1)
        all_q0_mu_stacked = torch.concat([rearrange(mu, '... c h w -> ... (c h w)') for mu in all_q0_mu], dim=-1)
        all_q0_logsig_stacked = torch.concat([rearrange(logsig, '... c h w -> ... (c h w)') for logsig in all_q0_logsig], dim=-1)
        all_z0_stacked = all_z0_stacked.reshape(batch_shape + all_z0_stacked.shape[1:])
        dist_z0 = Normal(all_q0_mu_stacked.reshape(batch_shape + all_q0_mu_stacked.shape[1:]), all_q0_logsig_stacked.reshape(batch_shape + all_q0_logsig_stacked.shape[1:]))
        log_qz0 = multiple_importance_logprob(all_z0_stacked, dist_z0, var_comp_neg_idx=-2)
        log_qz0 = log_qz0.reshape(B)
        ...
        return logits, log_q, log_p, log_qz0
