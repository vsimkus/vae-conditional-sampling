...
from .hps import HPARAMS_REGISTRY
...
class DecBlock(nn.Module):
    ...
    def sample_and_params(self, x, acts):
        qm, qv = self.enc(torch.cat([x, acts], dim=1)).chunk(2, dim=1)
        feats = self.prior(x)
        pm, pv, xpp = feats[:, :self.zdim, ...], feats[:, self.zdim:self.zdim * 2, ...], feats[:, self.zdim * 2:, ...]
        x = x + xpp
        z = draw_gaussian_diag_samples(qm, qv)
        # kl = gaussian_analytical_kl(qm, pm, qv, pv)
        params = {'qm': qm, 'qv': qv, 'pm': pm, 'pv': pv}
        return z, x, params

    def sample_and_pq_ratio(self, x, acts, use_dmmis=False, dmmis_num_comps=None):
        qm, qv = self.enc(torch.cat([x, acts], dim=1)).chunk(2, dim=1)
        feats = self.prior(x)
        pm, pv, xpp = feats[:, :self.zdim, ...], feats[:, self.zdim:self.zdim * 2, ...], feats[:, self.zdim * 2:, ...]
        x = x + xpp
        ...
        if use_dmmis:
            qm = rearrange(qm, '(b k) ... -> b k ...', k=dmmis_num_comps)
            qv = rearrange(qv, '(b k) ... -> b k ...', k=dmmis_num_comps)
            z_aug = rearrange(z, '(b k) ... -> k b 1 ...', k=dmmis_num_comps)

            q_distr = torch.distributions.Normal(qm, torch.exp(qv))

            var_logprob = q_distr.log_prob(z_aug).sum(dim=(-3, -2, -1))
            var_logprob = torch.logsumexp(var_logprob, dim=2, keepdim=True)
            var_logprob -= torch.log(torch.tensor(dmmis_num_comps, device=qm.device))
            var_logprob = rearrange(var_logprob, 'k b 1 ... -> (b k) ...')
        else:
            q_distr = torch.distributions.Normal(qm, torch.exp(qv))
            var_logprob = q_distr.log_prob(z).sum(dim=(1,2,3))

        pq_ratio = prior_logprob - var_logprob
    ...
    def forward_and_params(self, xs, activations, get_latents=False):
        x, acts = self.get_inputs(xs, activations)
        if self.mixin is not None:
            x = x + F.interpolate(xs[self.mixin][:, :x.shape[1], ...], scale_factor=self.base // self.mixin)
        z, x, params = self.sample_and_params(x, acts)
        x = x + self.z_fn(z)
        x = self.resnet(x)
        xs[self.base] = x
        if get_latents:
            return xs, dict(z=z.detach(), params=params)
        return xs, dict(params=params)

    def forward_and_pq_ratio(self, xs, activations, get_latents=False, use_dmmis=False, dmmis_num_comps=None):
        x, acts = self.get_inputs(xs, activations)
        if self.mixin is not None:
            x = x + F.interpolate(xs[self.mixin][:, :x.shape[1], ...], scale_factor=self.base // self.mixin)
        z, x, pq_ratio = self.sample_and_pq_ratio(x, acts, use_dmmis=use_dmmis, dmmis_num_comps=dmmis_num_comps)
        x = x + self.z_fn(z)
        x = self.resnet(x)
        xs[self.base] = x
        if get_latents:
            return xs, dict(z=z.detach(), pq_ratio=pq_ratio)
        return xs, dict(pq_ratio=pq_ratio)
...
class Decoder(HModule):
    ...
    def forward_and_params(self, activations, get_latents=False):
        stats = []
        xs = {a.shape[2]: a for a in self.bias_xs}
        for block in self.dec_blocks:
            xs, block_stats = block.forward_and_params(xs, activations, get_latents=get_latents)
            stats.append(block_stats)
        xs[self.H.image_size] = self.final_fn(xs[self.H.image_size])
        return xs[self.H.image_size], stats

    def forward_and_pq_ratio(self, activations, get_latents=False, use_dmmis=False, dmmis_num_comps=None):
        stats = []
        xs = {a.shape[2]: a for a in self.bias_xs}
        for block in self.dec_blocks:
            xs, block_stats = block.forward_and_pq_ratio(xs, activations, get_latents=get_latents, use_dmmis=use_dmmis, dmmis_num_comps=dmmis_num_comps)
            stats.append(block_stats)
        xs[self.H.image_size] = self.final_fn(xs[self.H.image_size])
        return xs[self.H.image_size], stats
...
def load_vae(H):
    vae = VDVAE(H)
    if H.restore_path:
        print(f'Restoring vd-vae from {H.restore_path}')
        restore_params(vae, H.restore_path, map_cpu=True)

    # ema_vae = VAE(H)
    # if H.restore_ema_path:
    #     print(f'Restoring ema vd-vae from {H.restore_ema_path}')
        # restore_params(ema_vae, H.restore_path, map_cpu=True)
    # else:
    ...
    print(f'{total_params=}, readable={total_params:,}')
