...
def discretized_mix_logistic_logprob(x, M, l, low_bit=False):
    # NOTE: copy of the above with just the last bit different
    """ log-likelihood for mixture of discretized logistics, assumes the data has been rescaled to [-1,1] interval """
    # Adapted from https://github.com/openai/pixel-cnn/blob/master/pixel_cnn_pp/nn.py
    xs = [s for s in x.shape]  # true image (i.e. labels) to regress to, e.g. (B,32,32,3)
    ls = [s for s in l.shape]  # predicted distribution, e.g. (B,32,32,100)
    ...
    log_probs = log_probs.sum(dim=3) + log_prob_from_logits(logit_probs)
    mixture_probs = torch.logsumexp(log_probs, -1)
    # NOTE: marginalise missing values
    return (mixture_probs*M.squeeze(-1)).sum(dim=[1, 2]) #/ np.prod(xs[1:])
...
class DmolNet(nn.Module):
    ...
    def log_prob(self, px_z, x, M):
        return discretized_mix_logistic_logprob(x=x, M=M, l=self.forward(px_z), low_bit=self.H.dataset in ['ffhq_256'])
    ...
    def sample(self, px_z):
        ...
        # Due to lossy downscaling of ffhq256 we need to rescale the outputs back to (-1, 1),
        # we do so stochastically by adding some noise.
        # low_bit=self.H.dataset in ['ffhq_256']
        # if low_bit:
        #     # im += torch.rand_like(im)*(1-0.9451)
        #     im += (1-0.9451)/2

        xhat = (im + 1.0) * 127.5
        # xhat = xhat.detach().cpu().numpy()
        # xhat = np.minimum(np.maximum(0.0, xhat), 255.0).astype(np.uint8)
        xhat = xhat.clamp_(0., 255.).to(torch.uint8)
        return xhat

    def sample_noquantization(self, px_z):
        # Outputs in (-1, 1)
        im = sample_from_discretized_mix_logistic(self.forward(px_z), self.H.num_mixtures)
        # low_bit=self.H.dataset in ['ffhq_256']
        # if low_bit:
